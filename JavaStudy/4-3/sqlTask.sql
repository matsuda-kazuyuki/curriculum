...
-- [回答]SELECT  store_table.store_name AS 店舗名, goods_table.goods_name AS 商品名, stock_table.quantity AS 在庫数
		FROM stock_table JOIN store_table ON stock_table.store_code = store_table.store_code
...
-- [回答]SELECT store_code, quantity FROM stock_table 
...
-- [回答]INSERT INTO goods_table
        (gooods_code, goods_name, price, update_day)
        VALUES('M001', 'マフラー', '2021-07-16')
 
 
-- 7. 在庫テーブルの商品コード='S987'、かつ、店舗コード='EA01'に対して、「在庫数=10、更新日付=本日日付」で更新しなさい。※実行後のSELECT結果も貼付すること。
-- [回答]UPDATE stock_table
        SET quantity = '10', update_day = '2021-7-18'
        WEHRE goods_code = 'S987' AND store_code = 'EA01'
